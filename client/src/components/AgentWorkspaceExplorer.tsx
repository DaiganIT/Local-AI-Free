...
import { useAgentFolderTree } from '#/hooks/useAgentFolderTree'
import { ExplorerRow } from '#/components/ExplorerRow'
import type { AgentFolderNode } from '#/lib/types'

interface AgentWorkspaceExplorerProps {
  agentId: string
  hostId: string
...
/**
 * Stateful: loads folder tree via react-query + measures panel for react-arborist.
 * Stateless visuals: explorer row renders icons + labels only.
 */
export function AgentWorkspaceExplorer({ agentId, hostId, openFilePath, onDeleteFile }: AgentWorkspaceExplorerProps) {
  const { data, isPending, isError, error, refetch, isFetching } = useAgentFolderTree(agentId)
